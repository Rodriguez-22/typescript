...
app.get('/items', (req, res) => {
  ...
  leerArchivoJson()
    ...
    .catch(error => res.status(500).json({ error: 'Error al leer el archivo' })); // Si hay un error, devolver un mensaje de error
});
...
app.post('/items', (req, res) => {
  ...
  leerArchivoJson()
    ...
    .catch(error => res.status(500).json({ error: 'Error al agregar el item' })); // Si hay un error, devolver un mensaje de error
});
...
app.delete('/items/:id', (req, res) => {
  ...
  leerArchivoJson()
    ...
    .catch(error => res.status(500).json({ error: 'Error al eliminar el item' })); // Si hay un error, devolver un mensaje de error
});
...
app.put('/items/:id', (req, res) => {
    ...
    leerArchivoJson()
      ...
      .then(() => res.json({ message: 'Item actualizado' }))
      .catch(error => res.status(500).json({ error: 'Error al actualizar el item' }));
  });
